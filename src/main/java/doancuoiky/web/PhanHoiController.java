...
public class PhanHoiController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/views/CTSV/phanhoi.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String maNguoiDung = request.getParameter("txtPhanHoiMaND");
        String maYC = request.getParameter("txtPhanHoiMaYC");
        try {
			yc = ycDao.getYeuCauCuThe(maNguoiDung, maYC);
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			response.getWriter().write("<script>alert('Error: " + e.getMessage() + "'); setTimeout(function(){ window.location.href='/DoAnCuoiKyDVSV_Nhom14_FINAL_2/views/CTSV/phanhoi.jsp'; }, 1000);</script>");
		}
        String maPH = "PH"+maYC+generateRandomID(2);
        String maNguoiPH = (String)session.getAttribute("MaND");
        String tieuDePH = request.getParameter("txtTieuDePH");
        String noiDungPH = request.getParameter("txtNoiDungPH");
        ...
        try {
            ...
            if (success) {
            	...
            	mailProcessor.send(ConstantData.sv01.getUsername(), tieuDePH, noiDungPH, ConstantData.ctv01.getUsername(), ConstantData.ctv01.getPassword());
            	mailProcessor.send(ConstantData.sv02.getUsername(), tieuDePH, noiDungPH, ConstantData.ctv01.getUsername(), ConstantData.ctv01.getPassword());
                response.getWriter().write("<script>alert('" + successMessage + "'); setTimeout(function(){ window.location.href='/DoAnCuoiKyDVSV_Nhom14_FINAL_2/views/CTSV/phanhoi.jsp'; }, 1000);</script>");
                
            } else {
                String failureMessage = "Reply failed!";
                response.getWriter().write("<script>alert('" + failureMessage + "'); setTimeout(function(){ window.location.href='/DoAnCuoiKyDVSV_Nhom14_FINAL_2/views/CTSV/phanhoi.jsp'; }, 1000);</script>");
                response.sendRedirect("/DoAnCuoiKyDVSV_Nhom14_FINAL_2/views/CTSV/phanhoi.jsp");
            }
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
            response.getWriter().write("<script>alert('Error: " + e.getMessage() + "'); setTimeout(function(){ window.location.href='/DoAnCuoiKyDVSV_Nhom14_FINAL_2/views/CTSV/phanhoi.jsp'; }, 1000);</script>");
